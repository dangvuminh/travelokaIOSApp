...
struct FlightSearchPannel: View {
    ...
    @State var popoverOpen = false
    var body: some View {
        VStack {
            ZStack {
                BottomLineControl(label: "From", icon: {
                    Image(systemName: "airplane")
                }, searchText: "Singapore (SIN)", onTap: {popoverOpen = true})
            ...
            BottomLineControl(label: "To", icon: {
                Image(systemName: "airplane")
            }, searchText: "Beijing (BEIA)", onTap: {popoverOpen = true})
            
            ZStack {
                BottomLineControl(label: "Departure time", icon: {
                    Image(systemName: "calendar")
                }, searchText: "02/02/2024", onTap: {print("Tap")})
                VStack {
            ...
            BottomLineControl(label: "Return time", icon: {
                Image(systemName: "calendar")
            }, searchText: "04/02/2024", onTap: {print("Tap")})
            HStack {
